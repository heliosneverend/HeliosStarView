...
@implementation ZYFCommentStarView
...
-(UIImage *)starImage_Full{
    if(!_starImage_Full){
        _starImage_Full = [UIImage imageNamed:@"byeCommentSecl.png"];
    }
...
-(UIImage *)starImage_Empty{
    if(!_starImage_Empty){
        _starImage_Empty = [UIImage imageNamed:@"byeCommentNorStar.png"];
    }
...
-(void)setup{
    ...
    for (int i=0; i<5; i++) {
        ...
        IMGV_Full.contentMode = UIViewContentModeScaleAspectFit;
...
-(void)ChoosedStar:(UIButton *)BTN{
    ...
    if(self.BYStarNumberBlock){
        self.BYStarNumberBlock(tag);
    }
}
